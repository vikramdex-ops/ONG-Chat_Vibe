...
from app.core.request_context import request_api_key, request_model, request_provider
...
def get_llm_provider(override_provider: Optional[str] = None) -> BaseLLMProvider:
    ...
    provider_type = (
        override_provider
        or request_provider()
        or current.llm_provider
        or "local"
    ).lower()
    user_key = request_api_key()
    user_model = request_model()

    if provider_type == "gemini":
        return GeminiLLMProvider(
            api_key=user_key or current.llm_api_key or current.gemini_api_key,
            model=user_model or current.llm_model_name or current.gemini_model_name,
        )
    if provider_type in ("openai", "ollama", "vllm"):
        return OpenAILLMProvider(
            server_url=current.llm_server_url,
            api_key=user_key or current.llm_api_key,
            model=user_model or current.llm_model_name,
        )
