    ...
    <div className="max-w-4xl mx-auto pb-12 space-y-6">
      ...
      <div className="panel p-6 space-y-4 border-2 border-emerald-400/60">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <label className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl border border-line bg-surface-muted text-xs font-semibold cursor-pointer">
            {kbBusy ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Upload className="w-3.5 h-3.5" />}
            Import KB zip
            <input
              type="file"
              accept=".zip"
              ...
              onChange={async (e) => {
                ...
                setKbMessage(null);
                try {
                  const res = await importKbPack(f);
                  setKbMessage(`Imported. ChromaDB path is now ${res.chroma_db_path}`);
                  const s = await getSettings();
                  setFormData(s);
                  if (onSettingsSaved) onSettingsSaved(s);
                } catch (err: any) {
                  setKbMessage(err.message || 'Import failed');
                } finally {
                  setKbBusy(false);
                }
              }}
        ...
        {kbMessage && (
          <div className="text-xs text-fg bg-surface-muted border border-line rounded-xl px-3 py-2">{kbMessage}</div>
        )}
      </div>
